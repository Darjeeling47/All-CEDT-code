...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      ...
      node *tmp_pos = mHeader -> next;
      for(int i = 0; i < pos; i++)
        tmp_pos = tmp_pos -> next;

      node *tmp_sel = mHeader -> next;
      int cnt = 0;
      for(int i = 0; i < mSize; i++){
        if(i == selected[cnt]){
          node *tmp_tmp = tmp_sel -> next;
          // set old node
          node *tmp_fnt = tmp_sel -> prev;
          node *tmp_bck = tmp_sel -> next;
          tmp_fnt -> next = tmp_bck;
          tmp_bck -> prev = tmp_fnt;
          //set new node
          tmp_pos -> prev -> next = tmp_sel;
          tmp_sel -> prev = tmp_pos -> prev;
          tmp_sel -> next = tmp_pos;
          tmp_pos -> prev = tmp_sel;
          // set new tmp
          tmp_sel = tmp_tmp;
          cnt++;
        }
